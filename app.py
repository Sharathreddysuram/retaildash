...
st.sidebar.header("\U0001F4C2 Data Loading")
if engine:
...
def load_data():
    try:
        ...
        st.sidebar.warning(f"⚠️ Falling back to CSVs due to: {e}")
...
with c2:
    st.metric("🛍️ Avg Spend/Basket", f"${avg_basket:,.2f}")
with c3:
    st.markdown(f"**As of:** {merged['DATE'].max().strftime('%b %d, %Y')}")

st.markdown("---")
st.subheader(f"📋 Data Pull for Household #{int(sel):04d}")
dfp = merged[merged[hcol]==sel].sort_values([hcol,bcol,"DATE",find_col(merged,"product_num"),"DEPARTMENT","COMMODITY"])
...
baskets = merged[merged[hcol]==sel].groupby(bcol)[find_col(merged,"product_num")].apply(lambda x: list(map(str,x))).tolist()
...
with st.expander("👪 Demographics & Engagement", expanded=True):
    ...
    ax.set_ylabel("Avg Spend/Visit")
    ax.legend(title="Has Children")
    fig.tight_layout()
    st.pyplot(fig)
...
with st.expander("📊 Engagement Over Time by Commodity"):
    ...
    ts_c = merged[merged["COMMODITY"].isin(top5)].groupby(
        [pd.Grouper(key="DATE", freq="MS"), "COMMODITY"]
    ...
    ax.legend(title="Commodity", ncol=2)
    fig.tight_layout()
    st.pyplot(fig)
